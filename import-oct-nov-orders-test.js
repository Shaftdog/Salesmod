/**
 * TEST VERSION - Import OCT-NOV Orders from CSV
 *
 * This script processes only the first 5 rows for testing
 */
...
const TEST_LIMIT = 5; // Only process first 5 rows
...
async function findOrCreateClient(clientName, email = null, phone = null) {
  ...
  if (existing) {
    console.log(`  ✅ Found existing client: ${existing.company_name}`);
...
async function findOrCreateProperty(address, city, state, zip, propertyType = 'single_family') {
  ...
  console.log(`  🏠 Property: ${street}, ${city}, ${state} ${zip}`);
...
async function processRow(row, index) {
  try {
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log(`📋 Row ${index + 1}: ${row['Appraised Property Address']}`);
    ...
    console.log(`  📅 Created: ${createdDate || 'N/A'}`);
    console.log(`  📅 Due: ${dueDate || 'N/A'}`);
 ...
 */
async function main() {
  console.log('🧪 TEST MODE - Processing first 5 rows only\n');
  ...
  try {
    ...
    console.log(`📊 Processing ${stats.totalRows} rows (TEST MODE)\n`);
  } catch (error) {
  ...
  console.log('\n✨ Test complete!\n');
  console.log('💡 If this looks good, run the full import with import-oct-nov-orders.js\n');
}
