...
interface DrillDownDealsTableProps {
  deals: Deal[]
}
...
export function DrillDownDealsTable({ deals }: DrillDownDealsTableProps) {
  ...
  const columns = useMemo<ColumnDef<Deal>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <Badge
              variant="outline"
              className={`${stageColors[stage] || 'bg-zinc-500/20 text-zinc-400'} border-0`}
            >
              {stage?.replace(/_/g, ' ') || '-'}
            </Badge>
          )
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-zinc-400">
            {formatDate(row.getValue('expectedCloseDate'))}
          </span>
        ),
      },
      {
