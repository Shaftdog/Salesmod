-- =============================================
-- Fix: Update transfer_contact_company function to include tenant_id in contact_companies
-- The INSERT was missing tenant_id which is a NOT NULL column
-- Created: 2025-12-09
-- =============================================

-- Drop and recreate the function with tenant_id in INSERT
...
BEGIN
  ...
  -- Create new company relationship (INCLUDING tenant_id!)
  INSERT INTO public.contact_companies (
    ...
    true,
    CURRENT_DATE,
    COALESCE(v_new_company_tenant_id, v_contact_tenant_id)
  )
  ON CONFLICT (contact_id, company_id, start_date)
  DO UPDATE SET
    is_primary = true,
    end_date = NULL,
    tenant_id = COALESCE(v_new_company_tenant_id, EXCLUDED.tenant_id),
    updated_at = NOW();
...
COMMENT ON FUNCTION public.transfer_contact_company IS
  'Atomically transfers a contact to a new company while preserving history. Includes tenant_id in contact_companies. Validates both belong to same tenant.';
