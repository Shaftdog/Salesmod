...
-- Note: bookable_resources doesn't have org_id, so we check via bookings
CREATE POLICY "Users can view GPS tracking in their org"
  ...
  USING (
    EXISTS (
      ...
      WHERE b.resource_id = gps_tracking.resource_id
      AND b.org_id = auth.uid()
    )
  );
...
-- API requests - rate limiting lookups (removed time-based predicate - not IMMUTABLE)
...
-- Note: audit_logs table exists with different schema from main app
-- Only create indexes that work with existing schema
DO $$
BEGIN
...
-- Note: Skipped because resource_utilization_summary doesn't exist in current schema

-- 2.3: Create function for concurrent materialized view refresh
-- Note: Skipped because resource_utilization_summary doesn't exist
...
-- Note: audit_logs comment skipped (table exists with different schema from main app)
