...
1. **Task status changes** → Updates job task counters
2. **Card state changes** → Updates job card metrics
3. **Cards created/deleted** → Recalculates totals
...
4. AGENT PROCESSES TASKS
   ├─> Creates kanban_cards (state: in_review)
   ├─> Links cards to job_id & task_id
   └─> Updates task status (done/error)
   ↓
5. USER REVIEWS CARDS
   ├─> Approves: state → approved
...
7. AGENT GENERATES NEXT BATCH (batch 1+)
   └─> Follow-up tasks based on results
   ↓
...
**Tables Created**: ✅ jobs, job_tasks
**Links Established**: ✅ kanban_cards.job_id, agent_runs.job_id
**Views**: ✅ job_metrics, job_performance_summary, recent_job_activity
...
The system is ready to orchestrate multi-step campaigns! 🚀
